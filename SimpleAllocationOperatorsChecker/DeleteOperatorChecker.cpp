...
namespace {
enum class AllocationStatus { None, Allocated, Deallocated };
...
public:
  ...
  bool isAllocated() const { return _status == AllocationStatus::Allocated; }
...
namespace {
class DeleteOperatorChecker
    : public Checker<check::PostStmt<CXXNewExpr>, check::PreStmt<CXXDeleteExpr>,
...
public:
  ...
  void checkPreStmt(const CXXDeleteExpr *expr,
                    CheckerContext &checkerContext) const;
  void checkPostStmt(const CXXNewExpr *expr,
                     CheckerContext &checkerContext) const;
  ...
  void checkPreStmt(const MemberExpr *expr,
                    CheckerContext &checkerContext) const;
};
} // namespace

REGISTER_MAP_WITH_PROGRAMSTATE(AllocationMap, SymbolRef,
                               AllocationStatusContext)
...
void DeleteOperatorChecker::checkPreStmt(const CXXDeleteExpr *expr,
                                         CheckerContext &checkerContext) const {
  ...
  if (isNullCondition.isConstrainedTrue()) {
    ReportBug(checkerContext, deleteCalledOnNull, "Delete called on a null.",
              true);
  } else if (isNullCondition.isUnderconstrained()) {
    // Reports warning when no nullability checks are done in code
    ReportBug(checkerContext, possibleDeleteCalledOnNull,
              "There's possibility of calling delete on a null.", false);
  }
  ...
  if (asc->isDeallocated()) {
    ReportBug(checkerContext, deleteCalledMultiple,
              "Delete called multiple times on the same memory location.",
              true);
  }

  // Set status of our argument symbol to deallocated and add thetransition to
  // the next state
  state = state->set<AllocationMap>(
      argSymbol, AllocationStatusContext(AllocationStatus::Deallocated));
  checkerContext.addTransition(state);
}

void DeleteOperatorChecker::checkPostStmt(
    const CXXNewExpr *expr, CheckerContext &checkerContext) const {
  ...
  state = state->set<AllocationMap>(
      symbol, AllocationStatusContext(AllocationStatus::Allocated));
  checkerContext.addTransition(state);
}

// Checks if instance methods are called on deallocated object
void DeleteOperatorChecker::checkPreCall(const CallEvent &call,
                                         CheckerContext &checkerContext) const {
  ...
  if (asc != nullptr && asc->isDeallocated()) {
    ReportBug(checkerContext, useAfterDelete, "Object used after delete.",
              true);
  }
}

// Check if we've left scope without deallocating memory
void DeleteOperatorChecker::checkDeadSymbols(
    SymbolReaper &symReaper, CheckerContext &checkerContext) const {
  ProgramStateRef state = checkerContext.getState();
  ...
  for (auto i = mapValues.begin(), end = mapValues.end(); i != end; ++i) {
    ...
    if (butDidYouDie && !i->second.isDeallocated() &&
        isNullCondition.isConstrainedFalse()) {
...
void DeleteOperatorChecker::checkPreStmt(const MemberExpr *expr,
                                         CheckerContext &checkerContext) const {
...
void DeleteOperatorChecker::ReportBug(CheckerContext &checkerContext,
  ...
  ExplodedNode *errorNode = sinkNode
                                ? checkerContext.generateErrorNode()
                                : checkerContext.generateNonFatalErrorNode();
  ...
  auto bugReport =
      std::make_unique<PathSensitiveBugReport>(*bugType, message, errorNode);

  checkerContext.emitReport(std::move(bugReport));
}

void DeleteOperatorChecker::ReportMemoryLeakBug(
    std::vector<SymbolRef> symbols, CheckerContext &checkerContext) const {
  ...
  auto bugReport = std::make_unique<PathSensitiveBugReport>(
      *resourceLeak, "You haven't released allocated memory.", errorNode);
...
void ento::registerDeleteOperatorChecker(CheckerManager &mgr) {
  mgr.registerChecker<DeleteOperatorChecker>();
}

bool ento::shouldRegisterDeleteOperatorChecker(const CheckerManager &mgr) {
  return true;
}
